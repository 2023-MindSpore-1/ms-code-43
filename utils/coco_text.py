...
class COCO_Text:
    ...
    def get_ids_in_TargetSet(self, TargetSet=""):
        if TargetSet == "val":
            return self.val
        if TargetSet == "test":
            return self.test
    ...
    def loadImgs_name(self, ids=[]):
        imgs_objects = self.loadImgs(ids)
        return [imgs_object['file_name'] for imgs_object in imgs_objects]
...
if __name__ == '__main__':
    api = COCO_Text("/old/wjf/test_CocoText/coco-text/COCO_Text.json")

    # 从标签找图片
    # AnnIds = api.getAnnIds()
    # id = AnnIds[4]
    # print(id)
    # # print(api.loadAnns(ids=id))
    # Ann = api.loadAnns(ids=id)
    # imgid = Ann[0]['image_id']
    # print(imgid)
    # print(api.loadImgs(ids=imgid))

    # 从图片找标签
    # ImgIds = api.getImgIds()
    # imgid = ImgIds[8]
    # print(imgid)
    # AnnIds = api.getAnnIds(imgid)
    # print(AnnIds)
    # Anns = api.loadAnns(AnnIds)
    # print(Anns)

    imgs_id = api.get_ids_in_TargetSet("val")
    # print(len(imgs_id))
    # AnnIds = api.getAnnIds(imgs_id)
    # print(len(AnnIds))

    # img = imgs_id[0]
    print(api.loadImgs(imgs_id[:10]))
    # for i in range(100):
    #     print(api.getAnnIds(imgs_id[i]))
    # annids = api.getAnnIds(imgs_id[:100])
    # anns  = api.loadAnns(annids)
    # print(anns)



    # ann_polygons = [ann["legibility"] for ann in anns]
    # import numpy as np
    # ann_polygons = np.array(ann_polygons)
    # print(ann_polygons)
